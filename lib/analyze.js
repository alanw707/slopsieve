...
function mdChecklist(findings) {
  ...
  } else if (findings.hasCodeChange && findings.testChangeRatio >= 0.05) {
    lines.push(`- [x] Test coverage present (${(findings.testChangeRatio * 100).toFixed(0)}% of changes are tests)`);
  }
  if (findings.hasDocsChange) {
    lines.push('- [x] Docs/README updated');
  } else {
